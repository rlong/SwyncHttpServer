...
-(JBJsonArray*)getFilesForDownload {
    ...
    for( NSUInteger i = 0, count = [mediaHandles getHandleCount]; i < count; i++ ) {
        ...
        NSString* uri = [NSString stringWithFormat:@"%@%@", [XPGetFileRequestHandler REQUEST_HANDLER_URI], [mediaHandle uriSuffix]];
        Log_debugString( uri );
...
-(JBBrokerMessage*)process:(JBBrokerMessage*)request {
    ...
    if( [@"getFilesForDownload" isEqualToString:methodName] ) {
        ...
        [associativeParamaters setObject:[XPHostnameUtilities getHostName] forKey:@"hostName"];
...
-(id)init {
    ...
    XPGetFileService* answer = [super init];
