...
@interface XPPostFilePartHandler ()
...
@implementation XPPostFilePartHandler
...
-(void)handleHeaderWithName:(NSString*)name value:(NSString*)value  {
    ...
    if( nil != contentDisposition ) {
        ...
        if( nil == filename ) {
            ...
            filename = [XPPostFilePartHandler removePathElementsFromFilename:filename];
            [self setFileName:filename];
            Log_debugString(filename);
            
            [self setUserError:[XPPostFilePartHandler validateFilename:filename]];
...
-(void)saveContentType  {
    ...
    [XPStorageMetaData saveContentType:contentType forFilename:_fileName];
...
-(id)initWithStorageManager:(id<XPStorageManager>)storageManager{
    ...
    XPPostFilePartHandler* answer = [super init];
